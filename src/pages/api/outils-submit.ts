...
import { depuisOutils } from '../../utils/entreesProspect';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    const entree = depuisOutils(payload);
    if (entree.ok) await creerDossier(entree.valeur, { origine: 'outils' });
    else console.warn('[outils-submit] Fiche non ouverte :', entree.erreur);
  } catch (err) {
    console.error('[outils-submit] Ouverture de la fiche impossible :', err);
  }
